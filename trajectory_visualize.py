...
ALGO = "SF"
PMPT = "elephant"
OUT_DIR = f"ddim_trajectory/ddpo_{ALGO}_aesthetics_trajectory/"
SOURCE_DIR = f"ddim_trajectory/ddpo_{ALGO}_aesthetics_pmpt_{PMPT}"

# Labels matching the prefixes you used when saving
# e.g. "baseline_img_step_t50.png", "ckpt1_img_step_t50.png", ...
CKPT_LABELS = ["t0", "t9", "t19", "t29", "t39", "t49"]

# Optional: where to save the comparison image
COMPARE_PATH = os.path.join(OUT_DIR, f"{ALGO}_compress_traj_prompt_{PMPT}.png")

TITLE = f"Denoise_trajectory_aesthetics_DDPO_{ALGO}_prompt_{PMPT}"
...
def find_image_for_label(source_dir, label):
    ...
    """
    pattern = os.path.join(source_dir, f"img_step_{label}.png")
